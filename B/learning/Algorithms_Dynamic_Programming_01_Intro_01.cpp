...
/*
Dynamic Programming (DP): is used in Max and Min and counting problems
...
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< how to make memoization like dr kosba said >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
int mem[MAX][MAX];	// R & C is in range N. Function return int <<<<< in this implementaion assume it is initialized some where else>>>>>>>>>>>>>
